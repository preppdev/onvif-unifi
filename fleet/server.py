...
# One-line appliance provisioning: GET /enroll/<token> returns a bootstrap script
# with the Tailscale key + enroll key + server URL baked in, so a box is set up
# with `curl -fsSL <fleet>/enroll/<token> | sudo bash`. The token gates the secrets.
PROVISION_TOKEN = os.environ.get("FLEET_PROVISION_TOKEN", "")
PROVISION_TSKEY = os.environ.get("FLEET_TAILSCALE_AUTHKEY", "")
PROVISION_PUBLIC_URL = os.environ.get("FLEET_PUBLIC_URL", "http://fleet:8080")
PROVISION_INSTALL_URL = os.environ.get(
    "FLEET_INSTALL_URL",
    "https://raw.githubusercontent.com/preppdev/onvif-unifi/main/install.sh",
)
...
# --------------------------------------------------------------------------- #
# one-line appliance provisioning
# --------------------------------------------------------------------------- #
@app.get("/enroll/<token>")
def enroll_script(token: str):
    if not (PROVISION_TOKEN and _ct_eq(token, PROVISION_TOKEN)):
        return Response("not found\n", 404, mimetype="text/plain")
    if not (PROVISION_TSKEY and ENROLL_KEY):
        return Response("# server missing FLEET_TAILSCALE_AUTHKEY / FLEET_ENROLL_KEY\n",
                        500, mimetype="text/plain")
    script = (
        "#!/usr/bin/env bash\n"
        "# ONVIF gateway appliance provisioning — generated by the fleet server.\n"
        "set -euo pipefail\n"
        f"curl -fsSL {PROVISION_INSTALL_URL} | \\\n"
        f"  TAILSCALE_AUTHKEY='{PROVISION_TSKEY}' \\\n"
        f"  FLEET_SERVER_URL='{PROVISION_PUBLIC_URL}' \\\n"
        f"  FLEET_ENROLL_KEY='{ENROLL_KEY}' \\\n"
        "  bash\n"
    )
